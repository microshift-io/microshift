...
- Clone the source repositories 
  ```bash
  git clone https://github.com/microshift-io/microshift.git microshift-upstream
...
- Build the container locally using podman
  ...
  - use `ovn-kubernetes` as CNI (default)
    ```bash
        sudo podman build -f microshift-upstream/microshift-okd-multi-build.Containerfile . -t microshift-okd
    ```
