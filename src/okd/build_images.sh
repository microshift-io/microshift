...
list_packages() {
  # WARNING: Keep this list synchronized with the 'images' array at line ~520
  # When adding a new image to the build, add its key to BOTH locations:
  #    1. Here (image_keys array)
  #    2. Line ~520 (images associative array)
  local image_keys=(
    "base"
    "cli"
    "haproxy-router-base"
  ...
  # Derive package names from image keys
...
# Other modes require exactly 4 arguments
if [[ $# -ne 4 ]]; then
  usage
fi
...
# WARNING: When adding a new image here, also update list_packages() at line ~418
declare -A images
